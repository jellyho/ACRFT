...
At each query the policy emits one chunk, the critic scores its prefixes in the same forward pass,
and the system executes the longest admissible prefix whose value is within $\epsilon$ of the best,
\begin{equation}
\label{eq:method-kappa}
\kappa(x, c) \;=\; \max\Big\{\,k \in \mathcal K \;:\; Q_\phi(x, c, k) \;\ge\;
\max_{k' \in \mathcal K} Q_\phi(x, c, k') - \epsilon \,\Big\}.
\end{equation}
The menu $\mathcal K \subseteq \{1,\dots,H\}$ of admissible commitments is a design choice, not
necessarily every length. A coarse menu costs little, because the values of neighbouring
commitments are close, while it shortens the range the actor must cover and keeps results
comparable to prior work that adapts over a small set. We report the menu with every result and
treat its size as an ablation rather than a fixed part of the method.
...
genuinely arrives (Corollary~\ref{cor:curriculum}). Apart from the menu, the comparison tolerance
$\epsilon$ is the only free parameter the commitment rule introduces, and no cost term is added to
the reward.

\subsection{Evaluation protocol}
\label{sec:method:protocol}

Two measurement rules follow from the way commitment comparisons fail, and we state them because
they change what the numbers mean rather than merely how they are computed.

First, the baseline is the \emph{best} constant commitment, not the default full chunk. The best
constant differs across tasks and cannot be known in advance, so comparing against the default
credits an adaptive rule with what a better constant already provides. We therefore measure the
whole fixed-length curve and report the margin against its maximum.

Second, every arm of a comparison is trained and evaluated within one submission, and the
uncertainty band comes from that comparison's own seeds. Run-to-run differences between separately
launched jobs can exceed seed variation, so a curve assembled from several submissions is not a
curve; and the seed spread of one comparison is not the ruler for another, since a band borrowed
from an unrelated contrast can make a real effect look like noise. Margins are reported as paired
per-seed differences with the sign count, and commitment length and success are reported as
separate measurements (Remark~\ref{rem:scope}).
