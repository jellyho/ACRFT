...
\begin{remark}[The sign of the distortion follows the reward convention]
\label{rem:sign}
...
convention, such as step penalties or negative cost to go, the same identity gives
$(\gamma-\gamma^{k})\,\mathbb E[V(s_k)]\le0$ and the distortion reverses, deflating long
commitments. Reports of horizon selectors collapsing toward short executions are therefore not in
tension with the proposition; beyond the sign flip, independent pressures push short even under
correct bookkeeping, since one more open-loop step weakly decreases the achievable value under any
data distribution~\citep{dqc}, so an idealized critic breaks ties toward requerying, conservative
targets penalize the larger open-loop variance of longer prefixes, and advantages at different
horizons live on $\gamma^{k}$-compressed scales that must be renormalized before they are
commensurable~\citep{aqc}. The bookkeeping distortion is a separate, convention-signed term, and
it is the one the backup \eqref{eq:smdp-backup} removes exactly, in either direction.
\end{remark}
