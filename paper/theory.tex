...
it, but composing one-step backups through the observed intermediate states integrates $U$ with
its marginal distribution, which chunk-level outcome regression structurally cannot. The states
where reaction is genuinely valuable are exactly the states where chunk regression is confounded,
since the same mid-window revelation creates both the reactive value and the confound.
\end{remark}
...
(iii) forms within-window values by composing backups through observed intermediate states rather
