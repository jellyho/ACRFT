...
if [ -f "$STATS" ] && [ "${FORCE_NORM_STATS:-0}" != "1" ]; then
  ...
  if ! uv run python scripts/compute_norm_stats.py --config-name "$CONFIG"; then
...
exec env XLA_PYTHON_CLIENT_MEM_FRACTION=1.0 uv run python scripts/train.py "$CONFIG" \
