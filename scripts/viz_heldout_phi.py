...
for panel, mode in enumerate(["episode", "progress"]):
    ...
    for k, hr in enumerate(hero_rows):
        ...
        if mode == "episode":
            ...
            lc = LineCollection(
                np.concatenate([seg[:-1], seg[1:]], axis=1),
                cmap="viridis",
    ...
    for sp in ax.spines.values():
        sp.set_color("#2a3140")
    ax.set_title(
        "HELD-OUT trajectories colored by "
        + ("EPISODE" if mode == "episode" else "PROGRESS")
        + "\n(ghost = TRAIN frames)",
        color="w",
        fontsize=12,
    )
fig.suptitle(
    "Generalization: 10 episodes NEVER seen by the TD loss, on the phi trained without them", color="w", fontsize=13
)
