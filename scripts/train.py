...
def train_step(
    ...
    def loss_fn(
        ...
        # Some models (e.g. Pi0AlphaFlow) return (per-sample loss, aux-metrics dict); the rest return
        # just the per-sample loss. Normalize to (loss, aux) so extra diagnostics are logged for free.
    ...
    info = {
        "loss": loss,
        # Emitted by every config so runs with and without an auxiliary loss are directly comparable
        # on one chart: for a plain BC model this is the whole loss, while for a model whose
        # top-level `loss` also carries an auxiliary term, only this key isolates the BC objective.
