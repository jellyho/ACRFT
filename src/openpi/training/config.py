"""openpi's own train configs. See ``_CONFIGS`` below for them, and ``config_acrft.py`` for the
ones this project adds; ``get_config(name)`` resolves against both."""
...
@functools.cache
def _configs_dict() -> dict[str, TrainConfig]:
    """The full registry: openpi's own configs above, plus the ones this project adds.

    The ACRFT configs (YAM, plus the RoboCasa sim configs the critic is scored against) live in
    their own module so that a rebase onto a new openpi only has to merge one file. That module
    imports the dataclasses above from this one, so
    it can only be imported once this module's body has run -- hence the import here rather than at
    the top. Being lazy also means either module can be imported first.
    """
    import openpi.training.config_acrft as _acrft_config

    configs = [*_CONFIGS, *_acrft_config.CONFIGS]
...
def get_config(config_name: str) -> TrainConfig:
    ...
    if config_name not in configs:
        closest = difflib.get_close_matches(config_name, configs.keys(), n=1, cutoff=0.0)
