...
def when(*paths):
    ...
    lo, hi = min(ts), max(ts)
    f = lambda t: _dt.datetime.fromtimestamp(t).strftime("%m-%d %H:%M")  # noqa: E731
...
def bars_with_ci(rows, *, width=620, rowh=30, pad_l=92, title_null=None, null_label=""):
    ...
    for i, (lab, k, n, tip) in enumerate(rows):
        ...
        out.append(f'<line x1="{xa:.1f}" y1="{y + 9}" x2="{xb:.1f}" y2="{y + 9}" stroke="var(--ink3)" stroke-width="1.5"/>')
        for xe in (xa, xb):
            out.append(f'<line x1="{xe:.1f}" y1="{y + 4}" x2="{xe:.1f}" y2="{y + 14}" stroke="var(--ink3)" stroke-width="1.5"/>')
...
def diverging(rows, *, width=620, rowh=28, pad_l=92):
    ...
    mx = max(1, max(max(a, b) for _, a, b, _ in rows))
    out = [f'<svg viewBox="0 0 {width} {h}" role="img">']
...
def lines(series, xs, *, width=620, height=210, pad_l=52, pad_b=34, ylab="", xlab="", logy=False, xticks=None):
    ...
    for i, xv in enumerate(xs):
        if xticks is None or xv in xticks:
            out.append(f'<text class="tick" x="{fx(i):.1f}" y="{height - pad_b + 14}" text-anchor="middle">{esc(xv)}</text>')
    for name, col, ys, emph in series:
        ...
        out.append(f'<polyline points="{" ".join(pts)}" fill="none" stroke="{col}" stroke-width="{w}" '
                   f'opacity="{op}" stroke-linejoin="round"/>')
        for i, v in enumerate(ys):
            if v is None:
                continue
            out.append(f'<g class="mark"><title>{esc(name)} · {esc(xs[i])} → {v:.4g}</title>'
...
def _embed(mp4: pathlib.Path, cache: pathlib.Path) -> str:
    ...
    if not small.exists():
        ...
        imageio.mimwrite(tmp, fr[::2], fps=10, codec="libx264",
...
def videos_for(run_dir, root, *, budget=None, cache=None):
    ...
    note = ("<p class='q' style='font-size:.8rem;color:var(--ink3)'>"
...
MODE_KO = {"vla": "vla — critic 없음 (기준)", "rand": "rand — 후보 무작위 선택",
           "bon": "bon — critic이 후보만 선택", "prefix": "prefix — critic이 커밋 길이만 선택",
...
def main() -> None:
    ...
    ap.add_argument("--embed-videos", type=int, default=0,
                    help="총 이만큼의 클립을 data URI로 임베드 (첫 run의 모드별 첫 클립부터). "
                         "클립당 약 1.3 MB. 0 = 링크만.")
    args = ap.parse_args()
    ...
    A("<div class='scroll'><table><thead><tr><th>표기</th><th>후보 선택</th><th>커밋 길이 선택</th>"
      ...
      "</tbody></table></div>")
    A("<p class='q' style='font-size:.85rem'>다섯 모드가 <b>같은 scene</b>(seed 고정)에서 돌므로 짝지어 비교됩니다. "
    ...
    A("<div class='scroll'><table><thead><tr><th>표기</th><th>무엇을 재나</th><th>기준값</th>"
    ...
    A(f"<p class='q' style='font-size:.85rem'><b>baseline</b> = {BASELINE_NOTE}.<br>"
    ...
    A("<p class='q' style='font-size:.85rem'><b>sweep 이름</b>: <code>fix_main</code> = 종료 부트스트랩을 "
      "플래그로 우회하던 시점의 16런. <code>mcdist</code> = 그 처리를 고친 뒤 돌린 "
      "MC하한 × 분포형 × 커버리지 2×2×2 교차 8런.</p>")

    A("<div class='note'><b>결과마다 얻은 시각이 제목 오른쪽에 붙어 있습니다.</b> "
      "이 조사는 코드를 고쳐가며 진행됐으므로, 시각이 다르면 코드 상태도 다를 수 있습니다 — "
      "해당하는 곳에 어느 상태였는지 적어뒀습니다.</div>")
    A("<div class='note'><b>이 페이지는 결론을 내지 않습니다.</b> 측정값과 각 축의 기준값(우연·영점)만 그립니다. "
      "두 개의 중심 지표는 해석이 모호하며, 그 모호함을 해당 차트 옆에 적어뒀습니다.</div>")

    # ---- 1. rollout 성공률 -----------------------------------------------------------------------
    A("<h2>1. rollout 성공률 — 실제 목적함수" + stamp(*runs_root.glob("*/*/rollout/*.json")) + "</h2>")
    A("<p class='q'>critic이 고른 것이 VLA가 그냥 낸 것보다 나은가. 다섯 모드가 <b>동일 scene</b>(seed 고정)에서 "
    ...
    for sweep_run in sorted(runs_root.glob("*/*/rollout")):
        ...
        if not agg:
            continue
        any_roll = True
        rows = [(MODE_KO.get(m, m), agg[m][0], agg[m][1], f"{m}: {agg[m][0]}/{agg[m][1]}")
                for m in MODE_ORDER if m in agg]
        base = agg.get("vla")
        ...
        A("<figure>" + bars_with_ci(rows, title_null=nullv, null_label="vla") +
    ...
    A("<p class='q'>성공률 차이는 표본에 흔들립니다. 같은 scene을 짝지어 <b>한쪽만 성공한 경우</b>만 세면 "
    ...
    for sweep_run in sorted(runs_root.glob("*/*/rollout")):
        ...
        for m in MODE_ORDER:
            ...
            if a or b:
                rows.append((MODE_KO.get(m, m).split(" — ")[0] + " — " + MODE_KO.get(m, m).split(" — ")[-1], a, b, f"{m}: vla만 {a}, {m}만 {b}, 나머지 {len(tr) - a - b}개는 동일"))
        if rows:
            A(run_head(runs_root, sweep_run.parent.parent.name, run,
                       f" <span style='color:var(--ink3);font-weight:400'>· {len(tr)} scene</span>"))
            A("<figure>" + diverging(rows) +
    ...
    if vb and vt:
        A("<h2>3. 값함수는 목표까지의 거리를 아는가" + stamp(runs_root / "vvd_base.json", runs_root / "vvd_tmc.json") + "</h2>")
        ...
        A("<figure>" + lines(
            ...
            xs, logy=True, ylab="Q (로그)", xlab="목표까지 남은 스텝") +
    ...
    for sw in ("fix_main", "mcdist"):
        ...
        A("<p class='q'>배포는 <code>(후보, prefix)</code> 결합 arg-max로 커밋 길이를 정합니다. "
        ...
        if ser:
            ...
            A("<figure>" + lines(ser, xs, ylab="평균 Q", xlab="prefix 길이 (스텝)") +
              "<figcaption>기울기가 음수 = 짧은 커밋 선호. <code>upc</code>가 평탄해지는지 보세요.</figcaption></figure>")
        rows = [[n, run_desc(runs_root, sw, n), f"{100 * (prof[n]['all'] or {}).get('frac_shortest', 0):.1f}%",
                 f"{(prof[n]['all'] or {}).get('mean_exec_steps', 0):.2f}",
                 f"{(prof[n]['all'] or {}).get('spearman_q_vs_prefix', 0):+.2f}"] for n in names]
        A("<div class='scroll'><table><thead><tr><th>run</th><th>baseline과 다른 점</th>"
          "<th>최단 prefix를<br>고른 비율</th><th>평균 커밋<br>(16 중 몇 스텝)</th>"
          "<th>ρ(Q, prefix)<br>−1이면 단조감소</th></tr></thead><tbody>"
          + "".join("<tr><td><code>" + esc(r[0]) + "</code></td><td style='white-space:normal;font-size:.8rem'>"
                    + esc(r[1]) + "</td>" + "".join(f"<td class='num'>{esc(c)}</td>" for c in r[2:]) + "</tr>"
                    for r in rows) + "</tbody></table></div>")

    # ---- 5. 오프라인 지표 ------------------------------------------------------------------------
    A("<h2>5. 오프라인 지표 — 그리고 왜 미덥지 못한가" + stamp(*runs_root.glob("*/*/diag.json")) + "</h2>")
    A("<div class='note'><b>이 두 지표는 'critic이 눈이 멀었다'와 '볼 것이 없다'를 구분하지 못합니다.</b><br>"
    ...
    for sw in ("fix_main", "mcdist"):
        ...
        A("<figure>" + bars_with_ci(
            [(f"{n} — {run_desc(runs_root, sw, n)[:46]}", int(round((a or 0) / mx * 1000)), 1000,
              f"{n}: act_sens {a:.5f} · {run_desc(runs_root, sw, n)}") for n, a, _ in rows],
            title_null=0.0, null_label="0 = 액션 무시") +
          "<figcaption>act_sens, 최댓값 기준 정규화. 오른쪽 숫자는 무시하고 막대 길이만 보세요 — "
          f"가장 큰 값이 {mx:.4f}입니다 (1.0이면 액션이 상태만큼 중요).</figcaption></figure>")
        A("<figure>" + bars_with_ci(
            [(f"{n} — {run_desc(runs_root, sw, n)[:46]}", int(round((b or 0) * 1000)), 1000,
              f"{n}: rank_cand {b:.3f} · {run_desc(runs_root, sw, n)}") for n, _, b in rows],
            title_null=0.5, null_label="우연 0.5") +
