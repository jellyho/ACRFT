...
def table(headers, rows, numeric_from=1):
    h = "".join(f'<th class="{"num" if i >= numeric_from else ""}">{html.escape(x)}</th>' for i, x in enumerate(headers))
    body = ""
    for r in rows:
        body += "<tr>" + "".join(
...
def main() -> None:
    ...
    for name in sorted(roll):
        m = roll[name]
        cell = lambda k: (f"{m[k][0]}/{m[k][1]} ({m[k][0] / m[k][1]:.0%})" if k in m else "—")  # noqa: E731
        pw, npair = _paired(name)
        pc = pw.get("critic", (0, 0, 1.0))
        roll_rows.append([
            f"<code>{html.escape(name)}</code>", f"{npair}", cell("vla"), cell("bon"), cell("prefix"), cell("critic"),
            f"{pc[0]}–{pc[1]} (p={pc[2]:.3f})",
        ])
    roll_tbl = table(
        ["run", "scene", "vla (critic 없음)", "bon (후보만)", "prefix (커밋길이만)", "critic (결합)",
         "vla vs critic 짝지은 승패"],
        roll_rows,
    ) if roll_rows else "<p class='meta'>rollout 결과 없음</p>"
    ...
    ceil_tbl = table(
        ["prefix h", "max target (=γ^h)", "", "valid transitions", "% of alive", "mean mc_return"], ceil_rows
    ) if ceil_rows else "<p class='meta'>prefix_bias_noprop.json 없음</p>"
    ...
    gamma_tbl = table(
        ["dataset (γ)", "γ^2/γ^16", "천장 @h=2", "천장 @h=16", "mean mc Δ(긴−짧은)"], gamma_rows
    ) if gamma_rows else ""
    ...
    sweep_tbl = table(
        ["run", "상태"] + [lbl for _, lbl, _ in keys], sweep_rows, numeric_from=1
    ) if sweep_rows else "<p class='meta'>아직 완료된 run 없음</p>"
    ...
    curve_tbl = table(
        ["run", "도달 step", "range/std 초기", "range/std 최종", "rank 초기", "rank 최종"], curve_rows
    ) if curve_rows else ""
    ...
    prof_tbl = table(
        ["run", "γ", "최단 선택(전체)", "평균 실행 step", "ρ(Q, prefix)", "최단 선택(목표근처)", "평균 step(목표근처)"],
        prof_rows,
    ) if prof_rows else "<p class='meta'>prefix_profile JSON 없음 — 학습 완료 후 생성</p>"
    ...
    A(f"<p class='sub'>sweep <code>{html.escape(args.sweep)}</code> · 완료 {done}/{len(runs)} run"
      + (f" · {html.escape(args.generated)}" if args.generated else "") + "</p>")

    A("<div class='box bad'><h4>요약</h4><ul>"
    ...
    A("<h3>1.1 환경</h3>")
    A("<pre><code>ROLLOUT=1 slurm/setup.sh          # venv, sim deps, RoboCasa 에셋 (로그인 노드, 1회)\n"
    ...
    A("<h3>1.2 데이터</h3>")
    A("<pre><code>slurm/fetch_data.sh            # jellyho/acrft-annot-noprop      -> $ANNOT_ROOT/noprop\n"
    ...
    A("<p><code>noprop</code> RLT 토큰은 proprio를 <b>의도적으로</b> 제외합니다(paper-faithful bottleneck). "
      "README는 그래서 “critic must supply proprio”라고 적지만, 공급하는 코드가 없었습니다 — "
      "<code>net.apply(params, token, action_chunk)</code>가 인자의 전부였습니다.</p>")
    A("<p>재-annotation(VLA 전체 재실행) 없이 복원했습니다: annotation이 <code>episode_index</code>/<code>frame_index</code>를 "
    ...
    A("<p>dim별 z-score를 적용합니다. raw state는 미터·쿼터니언·gripper qpos가 섞여 있고 critic의 첫 연산이 "
      "observation 전체에 대한 LayerNorm이라, 정규화 없이는 16 dim이 2048 토큰 dim의 통계에 짓눌립니다. "
      "std=0인 상수 dim 2개는 NaN 대신 0으로 통과시킵니다.</p>")
    A("<h3>1.4 학습</h3>")
    A("<pre><code>SWEEP=fix_main STEPS=20000 EVAL_EVERY=2000 SAVE_EVERY=10000 ROLLOUT=0 MAXPAR=8 \\\n"
      "  slurm/sweep.sh</code></pre>")
    A("<p>baseline에서 <b>한 번에 하나만</b> 바꾸는 OFAT입니다. baseline = ARQ · 후보 16개 hard max · ensemble min · "
    ...
    A("<pre><code>batch 256 · lr 3e-4 · 20,000 steps · seed 0 · ensemble 2 · target τ=0.005\n"
    ...
    A("<div class='box'><h4>batch 256인 이유 (RTX 3090 24576 MiB 실측)</h4>"
    ...
    A("<h3>1.5 평가</h3>")
    A("<pre><code>uv run scripts/eval_rlt_critic.py --data &lt;annot&gt; --params &lt;run&gt;/params.msgpack --out &lt;run&gt;/diag.json\n"
    ...
    A(table(["#","예측","근거","결과"],[
        ...
        ["E4","<code>upc</code>가 prefix 프로파일을 평탄하게 만든다",
         "커버리지 비대칭이 원인이라면 균일화로 사라져야 함",
         "<span class='good-t'>적중</span> — ρ −0.83 → <b>−0.07</b>"],
    ], numeric_from=99))
    A("<p class='meta'>E1–E2는 실행 전 예측이었고, 아래 3.1의 결함은 <b>예측에 없었습니다</b> — 진단 과정에서 발견했습니다.</p>")

    A("<h2>3. 실제 결과와 분석</h2>")

    A("<h3>3.1 결함 — 부트스트랩의 종료 처리가 두 규약을 섞어 썼다</h3>")
    A("<p>첫 sweep의 완료 런이 전부 같은 증상을 보였습니다: <code>q_demo_mean</code> ≈ 0.003인데 "
    ...
    if vvd_bad and vvd_ok:
        ...
        for b, o in zip(vvd_bad["bins"], vvd_ok["bins"], strict=False):
            rows.append([f"{b['lo']}–{b['hi'] if b['hi']<10**6 else '∞'}", f"{b['n']}",
                         fmt(b["mc"], ".4f"), fmt(b["q"], ".4f"), fmt(o["q"], ".4f")])
        A(table(["목표까지 step","n","참값 mc (γ^d)","Q — 수정 전","Q — 수정 후"], rows))
        A(table(["지표","수정 전 (30k step)","수정 후 (6k step)"],[
            ["corr(Q, mc_return)", fmt(vvd_bad["corr_q_mc"], "+.3f"), f"<b>{fmt(vvd_ok['corr_q_mc'],'+.3f')}</b>"],
            ["corr(Q, 거리)", fmt(vvd_bad["corr_q_dist"], "+.3f"), f"<b>{fmt(vvd_ok['corr_q_dist'],'+.3f')}</b>"],
            ["Q 평균 (참값 0.186)", fmt(vvd_bad["q_mean"], ".4f"), f"<b>{fmt(vvd_ok['q_mean'],'.4f')}</b>"],
        ]))
    A("<div class='box bad'><h4>원인: 규약 혼용</h4>"
      "<p>보상의 타이밍에는 자기정합적인 규약이 둘 있습니다.</p>"
      + table(["규약", "보상 합", "V(terminal)"],
              [["A — 도착 시 보상", "Σ<sub>i=1..h</sub> γ<sup>i−1</sup> r<sub>t+i</sub>", "0"],
               ["B — 상태의 보상 (<code>mc_return</code> 정의와 일치)",
                "Σ<sub>i=0..h−1</sub> γ<sup>i</sup> r<sub>t+i</sub>", "<b>r<sub>terminal</sub></b>"]],
              numeric_from=99)
    ...
    A("<div class='box bad'><h4>코드</h4>"
      ...
      "</div>")
    A("<div class='box good'><h4>수정: 종료 상태의 값 = 그 상태의 보상</h4>"
    ...
    A("<p>배포 규칙(<code>eval_critic.make_policy_fn</code>)은 <code>(candidate, prefix)</code>에 대한 <b>결합 arg-max</b>를 "
      "취하고 <code>(pp+1)·macro</code> step을 실행하므로, prefix 축을 따른 Q의 체계적 기울기가 커밋 길이를 결정합니다. "
      "annotation만으로(GPU 없이) 확인되는 구조적 비대칭이 있습니다.</p>")
    A(ceil_tbl)
    A("<p>terminal을 넘는 prefix는 transition이 생성되지 않으므로(<code>valid</code>), 목표에 가까울수록 살아남는 prefix가 "
    ...
    A("<p>3.1을 고친 뒤 측정한 값입니다. <code>ρ(Q, prefix)</code>가 −1에 가까울수록 Q가 커밋 길이에 대해 단조 감소합니다.</p>")
    A(prof_tbl)
    A("<div class='box good'><h4>인과 확인: <code>upc</code>가 기울기를 없앤다</h4>"
    ...
    A("<div class='box bad'><h4>예측 E3(γ)은 빗나갔습니다</h4>"
    ...
    A("<div class='box warn'><h4>읽는 법</h4>"
    ...
    A("<h4>지표 정의</h4>")
    A("<p>모든 지표는 <b>상태 내부</b>에서 계산합니다. <code>Q(z,a)=V(z)</code>로 붕괴한 critic도 TD loss와 "
      "Q-vs-return 상관에서는 멀쩡해 보이지만 후보를 무작위로 고르므로, 상태 간 지표는 판단 근거가 못 됩니다.</p>")
    A(GLOSSARY_TBL)
    A("<h4>각 run이 무엇인가</h4>")
    A(f"<p><b>baseline</b> = {BASELINE_NOTE}. 아래는 baseline과 <b>다른 점만</b> 적은 것입니다 — "
      "이 sweep은 한 번에 하나씩만 바꾸는 OFAT이므로, 적히지 않은 설정은 전부 baseline과 같습니다.</p>")
    _desc_rows = []
    for _n in sorted(runs):
        _c = runs[_n]["cfg"] or {}
        _d = describe(_c)
        _desc_rows.append([f"<code>{html.escape(_n)}</code>",
                           "<br>".join(html.escape(x) for x in _d) if _d else "<i>baseline</i>"])
    A(table(["run", "baseline과 다른 점"], _desc_rows, numeric_from=99))
    A("<p>판단 기준은 <code>act_sens</code>와 <code>rank_cand</code>입니다. 둘 다 기준값에 머물면 "
      "그 변종은 후보를 순위 매기지 못한 것이고, 나머지 열은 <i>어떻게</i> 실패했는지를 말합니다.</p>")
    A(sweep_tbl)
    if curve_tbl:
        A("<h4>학습 중 진단 추이 <span class='tag'>미완주 run 포함</span></h4>")
        A("<p><code>range/std</code>는 상태 내 Q의 (최대−최소)/자체 표준편차로, <b>후보 16개가 순수 노이즈면 ≈3.1–3.6</b>입니다.</p>")
        A(curve_tbl)

    A("<h3>3.5 sim rollout — 진짜 목적함수</h3>")
    A("<p>오프라인 지표는 전부 대리물입니다. 실제 질문은 “critic이 고른 것이 VLA가 그냥 낸 것보다 나은가”입니다. "
    ...
    A(roll_tbl)
    A("<div class='box bad'><h4>critic이 VLA를 이긴 적이 없습니다</h4>"
      ...
      "</div>")
    A("<div class='box warn'><h4>통계적으로 얼마나 말할 수 있나</h4>"
    ...
    A(table(["결함","증상","조치"],[
        ["<b>부트스트랩의 종료 처리가 규약 혼용</b>","성공 보상이 어떤 타깃에도 미진입 → 값함수가 0으로 붕괴, "
        ...
        ["<code>_diag</code>가 ARQ에서 크래시","<code>qd</code>에만 ensemble min 누락 → <code>[K,S,1,mh]</code>를 <code>[K]</code>로 축약. "
         "<code>--eval-every</code>가 처음 작동하는 step에서 <b>전 런</b>이 죽을 상황",
         "<code>qc</code>와 동일한 축약 순서로 수정, arq/qc × scalar/HL-Gauss 4조합 검증"],
        ["중간 체크포인트가 로드 불가","<code>config.json</code>이 학습 종료 시에만 기록 → <code>params_10000.msgpack</code>을 "
         "아무도 재구성 못 함. 동시 eval이 원천 차단",
         "첫 중간 저장 시점에 <code>config.json</code>도 기록"],
        ["critic이 proprio를 못 받음","<code>noprop</code> 토큰에도 없고 <code>Data</code>에도 없음",
         "<code>extract_proprio.py</code>로 원본에서 조인 복원(정합 검증 포함), 학습·평가·rollout 전 경로 연결"],
        ["rollout eval이 원본 LeRobot 데이터셋을 요구","VLA 생성 시 <code>data.create()</code>가 progress label 계산 위해 parquet reward 컬럼을 읽음",
         "<code>fetch_data.sh --lerobot</code> 추가 (0.64 GB)"],
        ["<code>big_qos</code>로는 job이 배치되지 않음","3개 job이 수 시간 <code>(Priority)</code> 대기",
         "<code>base_qos</code>(비선점, GPU 8 상한)로 전환하니 즉시 배치. resume 없는 이 학습에 비선점은 필수"],
        ["에셋이 quota 빠듯한 <code>/home</code>에 설치","RoboCasa가 <code>robocasa.__path__[0]/models/assets</code>로 해석 → repo 내부",
         "<code>textures</code>/<code>generative_textures</code>/<code>objects</code>를 <code>/scratch</code>로 심볼릭 링크"],
        ["<code>--bootstrap-candidates 16</code>이 no-op","코드가 <code>0 &lt; c &lt; N</code>일 때만 subsample. N=16이라 baseline과 동일한 런이 될 뻔",
         "<code>meta.json</code>에서 N을 읽어 N/2·N/4로 생성, 축퇴값은 skip"],
    ], numeric_from=99))

    A("<h2>부록 B — 재현 시 주의</h2>")
    A("<div class='box warn'><h4><code>slurm/env.sh</code>를 반드시 source</h4>"
    ...
    A("<div class='box'><h4>데이터가 거는 제약</h4>"
