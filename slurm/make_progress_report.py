...
def main() -> None:
    ...
    P(
        ...
        "무관하고, act_sens ≈ 0은 26개 런 전부에서 유지된다. IQL+QC의 rank_c 0.571도 롤아웃에서 "
        "bon−vla=0으로 실전 순위 능력이 아님이 판정됐다.</li>"
        "<li><b>프리픽스 축은 커밋 길이가 아니라 critic 오차를 재고 있었다.</b> 같은 상태의 프리픽스별 타깃은 "
        "이론상 전부 γ^d로 같아야 하는데, V 편향이 거리에 따라 달라 단조 감소했다 — 영상에서 보인 "
        "'prefix 값 단조 감소'의 정체.</li>"
        "<li><b>IQL이 프리픽스 축을 완치했다 (롤아웃 판정).</b> TD에서 가장 해로웠던 prefix 모드가 0.528 → "
        "<b>0.733 = vla 동급</b>. critic 전체 해악은 −0.285 → −0.142로 절반, expectile 용량-반응이 성공률에서도 "
        "재현(e50 −0.067 &lt; e90 −0.233, p=0.016). 남은 해악은 배포 best-of-N의 후보 arg-max 몫이며 데이터에 "
        "액션 대비가 없는 한 알고리즘으로 못 고친다 — 현 데이터의 안전한 배포는 <b>prefix-only</b>다.</li></ol>"
    )
    ...
    for t, ev in [
        ...
        (
            ...
            "오버레이 월드공간 확대로 재작성. 두 머신 작업 병합·push.",
        ),
        (
            "오늘 저녁",
            "IQL 롤아웃 판정: prefix 모드 0.528→0.733(vla 동급, 완치), critic 전체 해악 절반(−0.285→−0.142), "
            "용량-반응 재현(e90 p=0.016). iql_qc의 rank_c 0.571은 실전 미전이(bon−vla=0)로 기각.",
        ),
    ]:
    ...
    if ir:
        ...
        rollout_table(P, ir)
        P(
            "<div class='key win'><p><b>prefix 모드 완치: 0.528(TD) → 0.733 = vla 동급.</b> "
            "arg-max 제거 → 팽창 소멸 → 프리픽스 타깃 평탄화 → 커밋 길이 선택 정상화의 인과 사슬이 "
            "성공률 수준에서 완결됐다. critic 전체 해악은 절반(−0.285→−0.142), expectile 용량-반응 재현 "
            "(e50 −0.067 p=0.727 vs e90 −0.233 p=0.016 — τ는 낮게 두는 것이 맞다). bon은 그대로 나쁘다"
            "(0.650): 배포 best-of-N이 여전히 후보 16개에 arg-max를 걸고, 후보 간 참 가치 차이가 없는 한 "
            "그 arg-max는 잡음을 고른다. iql_qc의 rank_c 0.571은 bon−vla=0으로 실전 순위 능력이 아니었다."
            "</p></div>"
        )
    else:
