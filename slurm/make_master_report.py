...
<h3>중간 신호 — 다양화 프로브 부분 결과 (진행 중)</h3>
<p>canddiv 프로브(같은 48프레임에서 샘플링 세팅만 변경, v12 mixed critic으로 밴드 측정) 첫 두 세팅:
<b>flow steps 10→5만으로 후보 행동 스프레드 +29%(0.299→0.387 z-단위), critic Q밴드 +52%(0.025→0.038)</b>.
σ_signal이 고정 상수가 아니라 샘플링 노브로 열리는 양이라는 첫 증거 — 나머지 4세팅(노이즈 스케일 포함) 측정 중.
(운영 노트: 세팅마다 3B VLA를 재적재하다 GPU OOM 2회 — 단일 모델 + 샘플러 클로저 교체로 재구조화.)</p>
