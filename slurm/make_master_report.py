...
<tr><td>φ-128 / PCA-128</td><td colspan=4>측정 중 (같은 스크립트, 좌표계만 교체 — 워커A는 φ-공간에서 "전 구간 압승" 보고)</td></tr></table>
<p><b>1차 판독.</b> 원시 토큰 공간에서는 chunk 실행이 토큰을 어디로 보내는지의 대부분이 상태만으로 예측되고(0.653→0.636),
행동이 더해주는 정보가 +2.6%뿐이다 — 이대로 f를 쓰면 후보 간 착지점이 거의 같게 예측되어 선택이 다시 무력해진다.
워커A의 φ-공간 성공(binding .817)이 좌표계 문제임을 시사하므로, 같은 게이트를 φ/PCA 좌표에서 반복해
"행동 정보가 어느 표현에 사는가"를 확정한다.</p>
""",
)
...
필요하다. ③ proprio 유무는 무차이. ④ <b>critic 사다리 확정(n=8):</b> RLT2048 −0.065 / PCA-128 −0.040 / <b>φ-128 −0.010 CI[−0.078,+0.058],
...
과대추정된 꼬리 후보를 집는다. <b>v17b 확정(n=8): Δ̄=+0.052 CI[−0.016,+0.121], McNemar +74/−53 p=0.076 — 아직 null이나
프로젝트 사상 최고 위치.</b> CI 하한이 0에 −0.016까지 접근, 시드 8개 중 6개 ≥0, 무-veto 대비 페어드 순증
+21→+21(±). "다양화 + 오차-보수성" 쌍의 시그니처가 n=8에서도 유지 — <b>n=16 확정 판정 진행 중(시드 5800–6500
추가)</b>. 반면 φ 결합(v19)은 null: veto의 이득은 임베딩과 무관하게 선택 규칙에서 온다.</p>
