...
def _recorded_chunk_starts(reader: DatasetReader, episode: int, n_frames: int) -> list[int] | None:
...
def _recorded_candidates(reader: DatasetReader) -> int | None:
...
def _load_action_chunk(reader: DatasetReader, episode: int, start: int, horizon: int) -> np.ndarray | None:
...
def _load_agentview_extrinsics(config_path: str | None, arms: list[str]) -> dict:
    ...
    path = pathlib.Path(config_path) if config_path else pathlib.Path(__file__).parent / "agentview_extrinsics.yaml"
    try:
        extr = yaml.safe_load(path.read_text()) or {}
    except Exception as e:  # no snapshot -> no calibrated agentview, render the raw camera
        print(f"agentview: no usable extrinsics ({e}) -- agentview shown without an overlay", file=sys.stderr)
        return {}
    out = {}
    for arm in arms:
        ...
        if m is None:
            print(
                f"agentview: no calibrated extrinsic for '{arm}' in {path} -- skipping its agentview fan",
                file=sys.stderr,
            )
            continue
...
def _finite_prefix(path: np.ndarray) -> list | None:
...
def _load_full_candidates(reader: DatasetReader, episode: int, start: int, n_candidates: int) -> list | None:
...
def _value_series(reader: DatasetReader, episode: int, n_frames: int, n_candidates: int) -> tuple | None:
...
def _load_critic(reader: DatasetReader, episode: int, start: int, n_candidates: int) -> tuple[np.ndarray | None, int]:
...
def render(args: argparse.Namespace) -> pathlib.Path:
    ...
    # A kinematics-only copy of the YAM + linear-4310 model: bodies, joints and sites, with
    # the 7.4 MB of visual/collision meshes stripped. Verified to give bit-identical FK
    # (max |difference| 0.0 over 200 random configurations, whole WristCameraGeometry API).
    xml = str(pathlib.Path(__file__).parent / "models" / "yam_kinematics.xml")
