...
the page without editing anything here.
    ...
    uv run misc/reports/make_bias_report.py --out /scratch/jellyho/acrft/report_bias.html
"""
...
# NOTE: this script imports `_describe`, which was deleted from slurm/ in commit 3d8e8f5
# ("dead files removed"). It has not run since. Kept as the record of how its report was
# built; restore slurm/_describe.py from that commit's parent if you need to regenerate it.
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2] / "slurm"))
...
MODE_DESC = {
    ...
    "rand": (
        "무작위 후보",
        "후보 16개 중 하나를 균등 무작위로. critic을 쓰지 않으면서 VLA 기본값에서는 벗어난다 — 손실이 '다르게 골라서'인지 'critic이 골라서'인지 가르는 대조군.",
    ),
...
def bars_svg(rows, w=620, rowh=30, pad=110):
    ...
    for gx in range(0, 11, 2):
        ...
        out.append(
            f'<text x="{x:.1f}" y="{h - 6}" fill="var(--mut)" font-size="10" text-anchor="middle" '
    ...
    for i, (name, k, n) in enumerate(rows):
        ...
        out.append(
            f'<text x="{pad - 10}" y="{y + 11}" fill="var(--ink)" font-size="12" text-anchor="end" '
        ...
        out.append(
            f'<line x1="{x1:.1f}" y1="{y + 10}" x2="{x2:.1f}" y2="{y + 10}" stroke="var(--ink)" '
            f'stroke-width="1.2" opacity=".55"/>'
        )
        out.extend(
            f'<line x1="{xx:.1f}" y1="{y + 5}" x2="{xx:.1f}" y2="{y + 15}" stroke="var(--ink)" '
            f'stroke-width="1.2" opacity=".55"/>'
            for xx in (x1, x2)
        )
        out.append(
            f'<text x="{pad + inner + 8}" y="{y + 14}" fill="var(--mut)" font-size="11" '
...
def lines_svg(series, bands, w=620, h=280, pad_l=56, pad_b=44):
    ...
    for t in range(5):
        ...
        out.append(
            f'<text x="{pad_l - 8}" y="{Y(v) + 4:.1f}" fill="var(--mut)" font-size="10" text-anchor="end" '
            f'font-family="ui-monospace,monospace">{v:+.2f}</text>'
        )
    out.append(
        f'<line x1="{pad_l}" y1="{Y(0):.1f}" x2="{pad_l + iw}" y2="{Y(0):.1f}" stroke="var(--mut)" '
        f'stroke-dasharray="3 3"/>'
    )
    for i, (a, b) in enumerate(bands):
        out.append(
            f'<text x="{X(i):.1f}" y="{h - 20}" fill="var(--mut)" font-size="10" text-anchor="middle" '
            f'font-family="ui-monospace,monospace">{a}–{b}</text>'
        )
    out.append(
        f'<text x="{pad_l + iw / 2:.0f}" y="{h - 4}" fill="var(--mut)" font-size="10.5" '
    ...
    for name, vals, col in series_with_colors(series):
        ...
        out.append(
            f'<polyline points="{pts}" fill="none" stroke="{col}" stroke-width="1.9" '
            f'stroke-linejoin="round" stroke-linecap="round"/>'
        )
        last = [i for i, v in enumerate(vals) if v is not None][-1]
        out.append(
            f'<text x="{X(last) + 7:.1f}" y="{Y(vals[last]) + 4:.1f}" fill="{col}" font-size="10.5" '
...
def main() -> None:
    ...
    P(
        "<p class='lede'>ACRFT 오프라인 critic — v3_fixedmask 15런의 롤아웃 평가와, 그 결과를 설명하려고 "
        "돌린 세 가지 측정. 그리고 거기서 나온 IQL 구현.</p>"
    )
    P(
        f"<p class='stamp'>{html.escape(args.generated)} · PrepareCoffee · 시연 514 에피소드 / 279,534 프레임 · "
    ...
    for e in roll.values():
        for m in MODES:
            if m in e:
                ...
                tot[m][1] += e[m][1]
    P(
        "<div class='key'><p><b>결론부터.</b> critic에 결정권을 줄수록 성공률이 단조롭게 떨어집니다. "
    ...
    P("<h3>데이터</h3>")
    P(
        "<p>RoboCasa <code>PrepareCoffee</code>의 성공 시연 514 에피소드(279,534 프레임)를 π₀.₅ RLT "
    ...
    P(
        "<pre>uv run examples/robocasa/annotate_rlt.py \\\n"
    ...
    P(
        "<p>보상은 sparse입니다 — 성공 프레임 하나에만 1, terminal. 에피소드당 정확히 하나이고 "
    ...
    P("<h3>롤아웃 평가</h3>")
    P(
        "<p>학습과 <b>별도 잡</b>으로, 별도 GPU에서 돌립니다. 다섯 모드가 <b>같은 seed의 같은 장면</b>을 "
        "봅니다 — 그래야 비교가 성립합니다. 모드당 30 trial, 최대 1000 스텝.</p>"
    )
    P(
        "<pre>RUN_DIR=$CACHE_DIR/critic_runs/v3_fixedmask/&lt;run&gt; TRIALS=30 \\\n"
    ...
    P(
        "<div class='note'><p><b>이 다섯 모드가 왜 이렇게 짜였나.</b> critic은 두 가지를 결정합니다 — "
    ...
    P("<h2>2. 무엇을 예상했나</h2>")
    P(
        "<p>오프라인 진단에서 <code>action_sensitivity</code>가 15런 전부 0.0001~0.0014, "
        "<code>ranking_accuracy_demo_vs_candidate</code>가 전부 0.50±0.04였습니다. 둘 다 "
        "'critic이 액션을 구별하지 못한다'는 뜻이라, 예상은 이랬습니다:</p>"
    )
    P(
        "<ol><li><b><code>bon</code> ≈ <code>rand</code></b> — 순위가 무작위면 최고를 고르는 것과 아무거나 "
        "고르는 것이 같아야 한다.</li>"
    ...
    P(
        "<figure>"
        + bars_svg([(m, *tot[m]) for m in MODES])
    ...
    P(
        "<div class='note'><p><b>McNemar p</b> — 같은 장면을 두 모드가 각각 푼 <b>짝지은</b> 결과에 대한 "
    ...
    P(
        "<p>후보 16개의 참 가치가 사실상 같고 추정 오차만 다르면, arg-max는 <b>가장 좋은 후보가 아니라 "
    ...
    P(
        "<p>측정으로 뒷받침됩니다. 한 상태에서 후보 16개의 Q가 전부 <code>0.007</code> 폭 안에 들어 있는데 "
    ...
    P(
        "<pre>y_h = γ^h · V̂(s_{t+h}) = γ^h · (γ^{d-h} + b) = γ^d + γ^h·b\n"
        "                              ↑ h 무관, 참값      ↑ critic 오차</pre>"
    )
    P(
        "<p>V̂가 정확하면 <code>γ^h</code>이 상쇄되어 <b>프리픽스 8개의 타깃이 전부 같아야 합니다</b>. "
        "실제로는 그렇지 않습니다:</p>"
    )
    if pfx.get("buckets"):
        ...
        for b in pfx["buckets"]:
            cells = "".join(
                f"<td class='{'bad' if v > 1.15 else ('warn' if v > 1.03 else '')}'>{v:.4f}</td>" for v in b["ratio"]
            )
            P(f"<tr><td>{b['lo']}–{b['hi']}</td><td class='mut'>{b['n']}</td>{cells}</tr>")
        P("</tbody></table></div>")
        P(
            "<p class='mut' style='font-size:.9rem'>값은 <code>y_h / γ^d</code> — 1.0이면 이론과 일치, "
            "1보다 크면 과대추정. 각 구간 400상태.</p>"
        )
    P(
        "<p><b>두 가지가 동시에 보입니다.</b> 세로로 — 목표에서 멀수록 과대추정이 커져 250스텝 밖에서는 "
        "<b>5배</b>입니다. 가로로 — 모든 구간에서 h가 커질수록 비율이 <b>단조 감소</b>합니다. "
        "짧은 프리픽스가 먼 후속 상태에서 부트스트랩해 더 부푼 값을 물어오기 때문입니다.</p>"
    )
    P(
        "<p>그래서 배포의 <code>(후보, 프리픽스)</code> 결합 arg-max에서 <b>프리픽스 축이 커밋 길이가 아니라 "
    ...
    if vb:
        ...
        P(
            ...
            + "<th>critic 성공률</th></tr></thead><tbody>"
        )
        for r in sorted(vb):
            cells = ""
            for x in vb[r]["rows"]:
        ...
        P("</tbody></table></div>")
    P(
        "<div class='key'><p><b>여기서 제 가설이 깨졌습니다.</b> 편향이 성능을 설명할 것으로 봤는데, "
    ...
    P(
        "<p><code>cum</code>도 <code>V(s_{t+h})</code>도 <b>지금 평가하는 액션에 의존하지 않습니다</b>. "
    ...
    P(
        "<p>부트스트랩의 후보 max가 액션 정보를 넣어주는 것처럼 보이지만, 그것은 <b>다음 상태에서</b> 일어나 "
        "스칼라 하나로 접힌 뒤 들어옵니다. 현재 상태 안의 대비를 만들지 못합니다.</p>"
    )
    P(
        "<div class='note'><p><b>표현은 병목이 아닙니다.</b> RLT 토큰만으로 <b>본 적 없는 114개 에피소드</b>의 "
    ...
    P(
        "<p>지금까지의 대응(<code>topm</code>, <code>soft</code>, <code>lcb</code>, "
    ...
    P(
        "<pre>L_V = E[ |τ − 1(u&lt;0)| · u² ],   u = Q_target(z, a_demo, h) − V(z)\n"
        "L_Q = E[ (Q(z, a_demo, h) − y_h)² ],  y_h = cum_h + γ^h · ¬ended · V(z_{t+h})</pre>"
    )
    P(
        "<p>τ=0.5는 최소자승(V → 평균 Q, 개선 없음), τ가 클수록 초과분에 가중해 <code>max_a Q</code>에 "
        "접근합니다. 후보 forward가 사라져 <b>학습이 약 2배 빠릅니다</b>.</p>"
    )
    P("<pre>ROLLOUT=0 AXES=iql SWEEP=v6_iql slurm/sweep.sh</pre>")
    P(
        "<p>돌고 있는 것: <code>iql_e50</code> / <code>e70</code> / <code>e90</code> / <code>e95</code> "
    ...
    P(
        "<div class='note'><p><b>미리 말해둘 것.</b> IQL도 타깃이 액션에 무관하다는 근본 문제는 고치지 "
    ...
    for r in sorted(cfgs):
        d = describe(cfgs[r])
        P(
            f"<tr><td>{html.escape(r)}</td><td style='text-align:left;white-space:normal'>"
